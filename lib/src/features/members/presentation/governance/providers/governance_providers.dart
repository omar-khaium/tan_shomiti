...
final governanceMembersProvider = StreamProvider.autoDispose<List<Member>>((
  ref,
) async* {
...
final governanceRoleAssignmentsProvider =
    StreamProvider.autoDispose<List<RoleAssignment>>((ref) async* {
      ...
      yield* ref.watch(watchRoleAssignmentsProvider)(
...
final governanceUiStateProvider =
    Provider.autoDispose<AsyncValue<GovernanceUiState?>>((ref) {
      ...
      if (members.hasError) {
        return AsyncValue.error(
          members.error!,
          members.stackTrace ?? StackTrace.current,
        );
      }
      if (roles.hasError) {
        return AsyncValue.error(
          roles.error!,
          roles.stackTrace ?? StackTrace.current,
        );
      }
      if (consents.hasError) {
        return AsyncValue.error(
          consents.error!,
          consents.stackTrace ?? StackTrace.current,
        );
      }
