...
void main() {
  patrolTest('TS-502 e2e: sign-offs persist and status stays signed', (
    $,
  ) async {
    ...
    await $(#setup_name_field).enterText('TS502E2E Shomiti');
    ...
    for (final position in [1, 2, 3]) {
      ...
      await $.pumpAndSettle();
      await $(#payment_reference).enterText('trx-ts502-e2e-$position');
      await $(#payment_confirm).tap();
      await $.pumpAndSettle();
    }

    // Draw: run + record (walk months backwards until RunDrawPage appears).
    await $(#nav_more).tap();
    await $.pumpAndSettle();
    ...
    await $(#draw_proof_ref).enterText('vid-ts502-e2e-1');
    await $.pumpAndSettle();
    ...
    await $(#payout_proof_ref).enterText('trx-ts502-e2e-proof');
    await $.pumpAndSettle();
    await $(#payout_mark_paid).tap();
    await $.pumpAndSettle();

    // Statements: generate and sign-off.
    await $(#nav_more).tap();
    await $.pumpAndSettle();
    ...
    await $(find.text('Member 2')).tap();
    await $.pumpAndSettle();
    await $(#statement_signoff_proof).enterText('sig-ts502-e2e-w1');
    await $.pumpAndSettle();
    ...
    await $(find.text('Member 3')).tap();
    await $.pumpAndSettle();
    await $(#statement_signoff_proof).enterText('sig-ts502-e2e-w2');
    await $.pumpAndSettle();
    await $(#statement_signoff_save).tap();
    await $.pumpAndSettle();
    ...
    // Go back and verify sign-offs persisted ("View statement").
    ...
    await $(#statement_generate).tap(); // View statement
    await $.pumpAndSettle();
