...
void main() {
  patrolTest('TS-501 e2e: statement persists after generation', ($) async {
    ...
    await $(#setup_name_field).enterText('TS501E2E Shomiti');
    ...
    for (final position in [1, 2, 3]) {
      ...
      await $.pumpAndSettle();
      await $(#payment_reference).enterText('trx-ts501-e2e-$position');
    ...
    await $(#draw_proof_ref).enterText('vid-ts501-e2e-1');
    await $.pumpAndSettle();
    ...
    await $(#payout_proof_ref).enterText('trx-ts501-e2e-proof');
    await $.pumpAndSettle();
    await $(#payout_mark_paid).tap();
    await $.pumpAndSettle();

    // Statements: generate and verify details.
    await $(#nav_more).tap();
    await $.pumpAndSettle();
    ...
    // Go back and verify we can open the persisted statement ("View statement").
