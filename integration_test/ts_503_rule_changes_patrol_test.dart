...
void main() {
  patrolTest('TS-503: propose rule change + collect consent + apply', (
    $,
  ) async {
    ...
    await $(#setup_name_field).enterText('TS503 Shomiti');
    ...
    await $(#rule_change_shared_ref).enterText('chat-msg-ts503-1');
    ...
    for (final i in [0, 1, 2]) {
      ...
      await $.pumpAndSettle();
      await $(#rule_change_proof_reference).enterText('sig-ts503-$i');
      await $(Key('rule_change_consent_confirm_$i')).tap();
      await $.pumpAndSettle();
    }

    // Navigate back to rule changes and apply.
    await $(#nav_more).tap();
    await $.pumpAndSettle();
    await $(#more_rule_changes).scrollTo();
    await $(#more_rule_changes).tap();
