...
void main() {
  patrolTest('TS-305 (e2e): enforcement steps log audit and ledger cover', ($) async {
    ...
    // Setup with default thresholds (2 consecutive / 3 total).
    await $(#setup_name_field).enterText('TS305E2E Shomiti');
    ...
    // Generate dues for two months (trigger consecutive missed >= 2).
    await $(#nav_dues).tap();
    await $.pumpAndSettle();
    ...
    // Defaults: reminder -> notice -> apply.
    await $(#nav_more).tap();
    await $.pumpAndSettle();
    ...
    // Audit log should include the full sequence.
    await $(#nav_dues).tap();
    await $.pumpAndSettle();
    await $(#nav_more).tap();
    await $.pumpAndSettle();
    ...
    // Ledger should reflect the cover bookkeeping entry.
    await $(#nav_more).tap();
    await $.pumpAndSettle();
    ...
    expect(find.textContaining('Default cover applied'), findsWidgets);
  });
}
